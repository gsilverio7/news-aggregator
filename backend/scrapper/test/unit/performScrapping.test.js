...
const fs = require('fs');
const websiteFixture = fs.readFileSync('test/fixtures/ig-politica.html', 'utf8');
  ...
  it('should scrape data from a website', async () => {
    ...
    const mockBrowser = {
        newPage: jest.fn().mockResolvedValue({
          ...
          evaluate: jest.fn().mockImplementation((pageFunction) =>
    ...
    const resultWithoutDate = result.map(({ scrape_date, ...rest }) => rest);
